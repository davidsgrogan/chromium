...
void Element::RecalcStyle(const StyleRecalcChange change,
  ...
  if (IsDocumentElement()) {
    // Approach 2:
    // Use PaintLayerScrollableArea's and LayoutView::CalculateScrollbarModes's
    // existing complex scrollbar logic to create the real scrollbars so I can
    // read their thicknessess, by adding an extra call to
    // GetDocument().GetStyleResolver().PropagateStyleToViewport.
    if (GetComputedStyle() &&
        (ComputedStyleRef().OverflowX() == EOverflow::kScroll ||
         ComputedStyleRef().OverflowY() == EOverflow::kScroll)) {
      LOG(ERROR)
          << "Was going to do a new PropagateStyleToViewport so that "
             "LayoutView's "
             "PaintLayerScrollableArea::UpdateAfterStyleChange will call "
             "ComputeScrollbarExistence and create its scrollbars.";
      // GetDocument().GetStyleResolver().PropagateStyleToViewport(true);
      //
      // Issue: ... But this extra PropagateStyleToViewport causes bunch of
      // tests to fail, so I don't think it's safe. (See
      // https://crrev.com/c/7007591)
      //
      // Also, this approach means we wouldn't be using the modified viewport
      // units to resolve values on the root element. (Using the original
      // non-scrollbar-modified viewport units for values on the root element is
      // mentioned in the WG's resolution but isn't in the final spec. Point
      // being that maybe this limitation is ok.)
      //
      // Question: Do you think this approach of forcing early scrollbar
      // creation is promising and I should continue to hack up
      // StyleResolver::PropagateStyleToViewport to make the tests pass?
    }

    // Approach 3: mostly a proof-of-concept that seems very unlikely to be
    // sophisticated enough to evolve into a real solution. This approach's code
    // just assumes that overflow:scroll ALWAYS results in scrollbars and
    // subtracts what the theme says their size is. Except for custom
    // scrollbars, whose size is treated as 0.
    // This scaled-down approach passes the tests in this CL.

    // Issue: this approach ignores the complex existing logic I've been
    // mentioning in PaintLayerScrollableArea::ComputeScrollbarExistence and
    // friends, so it seems unlikely to be able to evolve to handle the
    // corner cases that we already have logic for. And, as it stands here,
    // this approach doesn't affect viewport units specified on the root
    // element. (But that small issue could easily be rectified by moving it to
    // StyleCascade::Apply())

    // Question: Am I wrong in thinking that
    // PaintLayerScrollableArea::ComputeScrollbarExistence and friends are so
    // sophisticated, and moving something simple like this to
    // StyleCascade::Apply could actually be sufficiently correct?
    // if (GetComputedStyle() &&
    //     !ComputedStyleRef().HasCustomScrollbarStyle(this)) {
    //   const Page* page = GetDocument().GetFrame()->GetPage();
    //   const ScrollbarTheme& theme = page->GetScrollbarTheme();
    //   const float scale_from_dip =
    //       page->GetChromeClient().WindowToViewportScalar(
    //           GetDocument().GetFrame(), 1.0f);
    //   const int scrollbar_thickness = theme.ScrollbarThickness(
    //       scale_from_dip, ComputedStyleRef().UsedScrollbarWidth());
    //   if (ComputedStyleRef().OverflowX() == EOverflow::kScroll) {
    //     GetDocument().GetLayoutView()->vertical_scrollbar_width =
    //         scrollbar_thickness;
    //   }
    //   if (ComputedStyleRef().OverflowY() == EOverflow::kScroll) {
    //     GetDocument().GetLayoutView()->horizontal_scrollbar_height =
    //         scrollbar_thickness;
    //   }
    //   // CSSToLengthConversionData::ViewportSize::ViewportSize reaches out to
    //   // LayoutView, which will subtract these stored scrollbar sizes.
    //   GetDocument().GetStyleEngine().UpdateViewportSize();
    // } else {
    //   GetDocument().GetLayoutView()->vertical_scrollbar_width = 0;
    //   GetDocument().GetLayoutView()->horizontal_scrollbar_height = 0;
    //   GetDocument().GetStyleEngine().UpdateViewportSize();
    // }
  }
