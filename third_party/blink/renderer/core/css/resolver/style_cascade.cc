...
void StyleCascade::Apply(CascadeFilter filter) {
  ...
  if (IsRootElement() && !state_.IsForPseudoElement()) {
    ...
    PhysicalSize to_subtract =
        view->GetScrollableArea()->ComputeScrollbarExistence2(
            some_scroll_properties);
    LOG(ERROR) << "Setting view->vertical_scrollbar_width = "
               << to_subtract.width.ToInt();
    view->vertical_scrollbar_width_for_viewport_units =
        to_subtract.width.ToInt();
    view->horizontal_scrollbar_height_for_viewport_units =
        to_subtract.height.ToInt();
